...
modules/
  ...
  tailscale.nix      # core.tailscale.enable — daemon; tunnel bound to the admin's login session
...
- `core.nymvpn.enable` — NymVPN daemon and CLI, machine-wide: one account
  ...
  user's traffic.
- `core.tailscale.enable` + `core.tailscale.user` — tailscale daemon, with
  the tunnel up only while that user is logged in, and never with an exit
  node: only tailnet destinations use it, everything else goes via NymVPN.
  Which tailnet (and any keys) lives in a root-owned
  `/etc/tailscale/connect.env`, never in the repo or nix store.
