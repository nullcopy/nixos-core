...

{
  ...
  core.tailscale.enable = true;     # tailnet, up while alice is logged in — see step 6
  core.tailscale.user = "alice";
...
| `core.tailscale.enable` + `core.tailscale.user` | tailscale daemon; the tunnel is up only while `user` is logged in, never with an exit node; tailnet policy read from a root-owned env file, never the repo (step 6) |
...
2. Clone `MACHINE_REPO` to `/mnt/home/<ADMINUSER>/.nixos`, generate the
   real `hardware-configuration.nix` into it, and commit it (as
...
4. Run `nixos-install` (this builds/downloads the whole system — expect a
   while on first install, and note it also generates `flake.lock`,
   pinning nixos-core + nixpkgs for this machine), prompt for
   `ADMINUSER`'s login password, hand the repo's ownership to
   `ADMINUSER`, and unmount.
...
```sh
cd ~/.nixos
git add -A                            # flake.lock (generated by nixos-install) + any edits
git commit -m "Pin inputs"
git push                              # this + the installer's hardware-config commit
...
- **tailscale** carries *only* tailnet destinations (`100.64.0.0/10` and
  MagicDNS names) — it is never given an exit node — and is up only while
  `core.tailscale.user` has a login session. Other users never see it and
...
**tailscale** (`core.tailscale.enable` + `core.tailscale.user`): the repo
only ever names the user; *which* tailnet, and any keys, live in a
root-owned file outside both the repo and the nix store, so nothing
sensitive can leak through git or world-readable store paths. Create it
exactly like this — bare `KEY=VALUE` lines, no trailing comments (systemd
does not strip them):
...
Then enroll the machine once — either interactively:

```sh
sudo tailscale up --login-server=https://your.headscale.example
```

…or unattended, by adding a pre-auth key line (`TAILSCALE_AUTH_KEY=tskey-…`,
from `headscale preauthkeys create`) to the file before the admin's first
login. Either way, from then on `tailscale-connect@<uid>` applies the
policy from that file (with `--reset`, so the file is the whole policy)
when the admin logs in and runs `tailscale down` when their last session
ends. Log out and back in after changing the file, or
`sudo systemctl restart "tailscale-connect@$(id -u)"`.

Note `loginctl enable-linger` keeps a user's session manager alive after
logout — for the admin that also keeps tailscale up.
